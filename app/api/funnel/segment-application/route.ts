...
/* Each ads-gate segment gets its own table so leads/conversion can be
 * measured independently per segment (mirrors why /funnel/vsl uses its own
 * `vsl_applications` table instead of sharing `funnel_leads`). */
const SEGMENT_TABLES: Record<string, string> = {
  'under-100k':       'ads_under_100k_applications',
  'over-100k-ads':    'ads_over_100k_ads_applications',
  'over-100k-no-ads': 'ads_over_100k_noads_applications',
};
...
export async function POST(req: Request) {
  ...
  const table = SEGMENT_TABLES[segment];
  if (!table) return NextResponse.json({ error: 'invalid segment' }, { status: 400 });
  if (!email) return NextResponse.json({ error: 'email required' }, { status: 400 });

  // Partial saves (each step before the final one) capture drop-offs. Only the
  // final step sends completed:true and triggers the CRM lead + GHL contact.
  const isComplete = completed === true;

  /* 1. Upsert into the segment's own Supabase table (JSON drops undefined keys,
     so a partial save only writes the fields answered so far). */
  const { data: appRow, error: dbErr } = await db()
    ...
    .upsert({
      ...
      qualified: isComplete ? qualified : undefined,
      source: source || `ads-${segment}`,
  ...
  if (dbErr) console.error(`[segment-application:${segment}] supabase:`, dbErr.message);

  // Partial save — persist answers-so-far and stop (no CRM/GHL noise per step).
  if (!isComplete) {
    return NextResponse.json({ ok: true, id: appRow?.id ?? null, segment, partial: true });
  }
  ...
  try {
    ...
    if (igHandle) {
      const { data } = await db().from('crm_leads').upsert(crmRecord, { onConflict: 'ig_handle' }).select('id').maybeSingle();
      leadId = data?.id ?? null;
    } else {
    ...
    queueCloseSync(leadId, `segment-application:${segment}`);
    queueAlowareSync(leadId, `segment-application:${segment}`);
  } catch (crmErr) {
    console.error(`[segment-application:${segment}] CRM upsert error:`, crmErr);
  }
  ...
  const sourceTags: string[] = [`segment-${segment}`];
  if (traffic_source) sourceTags.push(`src-${traffic_source}`);
  if (utm_campaign)   sourceTags.push(`camp-${String(utm_campaign).slice(0, 40)}`);
  ...
  try {
    await fetch(`${GHL_API}/contacts/`, {
      ...
      body: JSON.stringify({
        ...
        customFields: [
          ...
          { key: 'funnel_source',        field_value: `ads-${segment}` },
          { key: 'funnel_segment',       field_value: segment },
          /* attribution */
    ...
    console.error(`[segment-application:${segment}] GHL error:`, e);
  }

  return NextResponse.json({ ok: true, id: appRow?.id ?? null, segment, qualified, traffic_source });
}
